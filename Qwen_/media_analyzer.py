...
Default model: Qwen/Qwen3-VL-2B-Instruct (~4-5GB in bf16, fits comfortably
on a 16GB GPU with headroom for image tokens, KV cache, and batching).
...
# flash-attn is optional but recommended if your GPU supports it:
...
NOTE ON PIPELINE USE
---------------------
If you're wiring this into a larger pipeline (e.g. video_indexer_main),
instantiate `QwenVLAnalyzer` ONCE at startup and reuse it across every
video/frame you process. Do NOT create a new analyzer per call — reloading
model weights repeatedly is slow and wastes GPU memory churn. The
`analyze_media()` function below now accepts an optional pre-built
`analyzer` argument for exactly this reason.
"""
...
#   "Qwen/Qwen3-VL-8B-Instruct"        -> bigger dense model, needs ~16GB+ VRAM
#   "Qwen/Qwen3-VL-30B-A3B-Instruct"   -> MoE, ~60GB+ VRAM to hold all experts
...

{
  ...
  "counts": {
    "total_people": "integer, total number of people visible",
    "total_animals": "integer, total number of animals visible",
    "specific_objects": {"object_name": "integer count, e.g., 'car': 3, 'chair': 4"}
  },
...
Rules:
- If a category has nothing to report, use an empty list / null / 0, don't omit the key.
- Provide accurate integer counts in the "counts" section.
...
class QwenVLAnalyzer:
    ...
    def analyze_image(
        ...
        # Free intermediate tensors and clear CUDA cache so repeated
        # frame-by-frame calls don't accumulate memory over a long video.
...
def analyze_media(
    ...
    max_frames: int = 40,
    scene_change_mode: bool = False,
    analyzer: Optional["QwenVLAnalyzer"] = None,
) -> dict:
    ...
    # --- ADD THIS HELPER FUNCTION INSIDE analyze_media ---
    def add_computed_counts(analysis_dict: dict):
        if not isinstance(analysis_dict, dict) or "parse_error" in analysis_dict:
            return
        
        # Count items in the 'people' list
        if "people" in analysis_dict and isinstance(analysis_dict["people"], list):
            analysis_dict["computed_people_count"] = len(analysis_dict["people"])
            
        # Count items in the 'objects' list
        if "objects" in analysis_dict and isinstance(analysis_dict["objects"], list):
            analysis_dict["computed_object_count"] = len(analysis_dict["objects"])
            
            # Group and count by object label
            label_counts = {}
            for obj in analysis_dict["objects"]:
                label = obj.get("label", "unknown").lower()
                label_counts[label] = label_counts.get(label, 0) + 1
            analysis_dict["computed_object_breakdown"] = label_counts
    # -----------------------------------------------------
...
def main():
    ...
    # Build the analyzer once here. If you extend main() to loop over
    # multiple inputs, pass this same analyzer instance into analyze_media()
    # for each one instead of letting it construct a new one per file.
    analyzer = QwenVLAnalyzer(model_id=args.model)
