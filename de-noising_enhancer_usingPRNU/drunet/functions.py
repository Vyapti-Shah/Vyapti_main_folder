...
import numpy as np
import torch
from drunet import utils_model
from drunet import utils_image as util
...
def noise_extract_drunet(im: np.ndarray, levels: int = 100, sigma: int = 100) -> np.ndarray:
    ...
    model_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', model_name + '.pth'))
    ...
    img_L = torch.cat(
        (img_L, torch.FloatTensor([noise_level_img / 255.]).repeat(1, 1, img_L.shape[2], img_L.shape[3])), dim=1)
    ...
    noise = cv2.subtract(img_H, img_E)
